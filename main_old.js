  ...
  if (customCursor) {
    ...
    function updateCursorFromEvent(e) {
      ...
      const newScale = Math.max(
        0.7,
        BASE_CURSOR_SCALE - Math.min(0.5, speed * 0.02)
      );
    ...
    try {
      ...
      if (el && el.tagName === "CANVAS") {
        try {
          ...
          const cx = Math.floor(
            (thumbCenterX - crect.left) * (el.width / crect.width)
          );
          const cy = Math.floor(
            (thumbCenterY - crect.top) * (el.height / crect.height)
          );
          // Request a 2d context optimized for readback (avoids console warning)
          const ctx = el.getContext("2d", { willReadFrequently: true });
          if (ctx) {
            const d = ctx.getImageData(
              Math.max(0, Math.min(el.width - 1, cx)),
              Math.max(0, Math.min(el.height - 1, cy)),
              1,
              1
            ).data;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const words = target.querySelectorAll(
    ":scope > h1 .word, :scope > h2 .word, :scope > h3 .word, :scope > span .word"
  );
  if (words.length) {
    gsap.to(words, {
      y: 0,
